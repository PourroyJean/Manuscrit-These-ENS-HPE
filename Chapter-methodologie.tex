...
%%%%%%%%%%%%%%%%%
Les performances étant généralement limitée par la bande passante mémoire, l'outil YAMB (voir \autoref{sec:YAMB}) peut être utilisé pour récupérer le profile du bus mémoire. On peut ainsi vérifier si le bus est bien saturé et comparer les ratio de lecture/écriture avec ceux mesurée lors de l'étape 3 présentée dans la \autoref{sec:smm}. Grâce à ces ratios, on peut déterminer si le jeu de donnée est lu plusieurs fois, et si des techniques de \textit{blocking} pour améliorer la localité dans la mémoire cache doivent être utilisées.
